...
```
rgName=`az group show --name ${{ secrets.rg }} --query location --output tsv`
```
...
```
az monitor app-insights component create --app apimon${{ secrets.webapp }} --location $rgName --kind web -g ${{ secrets.rg }} --application-type web --retention-time 30
```
